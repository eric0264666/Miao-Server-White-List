  ...
  try {
    ...
    try {
      ...
      if (res.data.code === 200) {
        success.value = true
        successMessage.value = res.data.msg || '验证成功,请等待猫猫服管理员审核!'
      } else {
      ...
      try {
        ...
        if (res.data.code === 200) {
          success.value = true
          successMessage.value = res.data.msg || '验证成功,请等待猫猫服猫猫服管理员审核!'
        } else {
          error.value = res.data.msg || '验证失败'
        }
      } catch (verifyErr) {
        ...
        if (verifyErr.message && verifyErr.message.includes('Network Error')) {
          error.value = '网络连接错误，请检查网络设置或联系猫猫服管理员'
        } else if (verifyErr.message && verifyErr.message.includes('CORS')) {
          error.value = '跨域请求错误，请联系猫猫服管理员配置服务器访问权限'
        } else {
